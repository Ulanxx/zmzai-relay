        ...
        <CardSpotlight key={model.model} radius={260} color="rgba(196, 42, 36, 0.10)">
          <div className="flex h-full flex-col p-5">
            <div className="flex items-center justify-between gap-3">
              <p className="font-mono text-lg">{model.model}</p>
            ...
            <Link href={`/models/${encodeURIComponent(model.model)}`} className="mt-4 font-mono text-xs text-accent-readable underline underline-offset-4">查看详情 →</Link>
          </div>
        </CardSpotlight>
      ))}
    </div>
