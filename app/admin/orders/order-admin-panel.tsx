...
export function OrderAdminPanel({ initialOrders }: { initialOrders: Order[] }) {
  ...
  return (
    <div>
      <p className="text-sm text-muted">确认前请以微信/支付宝实际到账记录为准，截图只能作为辅助。确认后会立即增加用户余额，且不能重复确认。</p>
      {orders.length === 0 ? (
        ...
        <ul className="mt-6 divide-y divide-line rounded-lg border border-line bg-bg">
          {orders.map((order) => (
            <li key={order._id} className="p-4">
              ...
              <p className="mt-3 font-mono text-xs text-muted">收款 {cny(order.paymentAmountFen)} · 到账 {cnyMicrosLabel(order.creditMicros, 2)} · {order.paymentMethod === "wechat" ? "微信" : "支付宝"}</p>
              ...
              {order.status === "submitted" ? (
                <div className="mt-4 flex gap-3">
                  ...
                  <Button type="button" variant="danger" size="sm" disabled={busy === order._id} onClick={() => review(order, "reject")} className="disabled:opacity-50">驳回付款</Button>
                </div>
              ) : order.reviewNote ? (
