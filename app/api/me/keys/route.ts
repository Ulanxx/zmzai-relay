...
export async function POST(req: NextRequest) { const user = await getCurrentUser(); if (!user) return unauthorized(); const parsed = createSchema.safeParse(await req.json().catch(() => null)); if (!parsed.success) return NextResponse.json({ error: "Token 配置格式不正确", code: "INVALID_BODY" }, { status: 400 }); const created = generateApiKey(); await connectMongo(); const record = await ApiKeyModel.create({ ...parsed.data, ...created, userId: user.id, status: "active", quotaTotalTokens: 0, quotaUsedTokens: 0 }); return NextResponse.json({ key: created.plaintext, record: { _id: String(record._id), prefix: record.prefix, name: record.name, status: record.status } }, { status: 201 }); }
