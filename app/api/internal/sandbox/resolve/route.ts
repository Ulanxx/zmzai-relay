...
export async function POST(request: NextRequest) {
  ...
  const key = await resolveSandboxKey(parsed.data.sandboxKey);
  if (!key) return NextResponse.json({ code: "SANDBOX_KEY_INVALID", error: "Sandbox key 无效或已撤销" }, { status: 401 });
  return NextResponse.json({ keyId: key.id, userId: key.userId, name: key.name }, { headers: { "Cache-Control": "no-store" } });
}
