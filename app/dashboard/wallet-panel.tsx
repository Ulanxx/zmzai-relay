...
export function WalletPanel({ products, initialOrders, qrUrl, accountName, notice }: { products: Product[]; initialOrders: Order[]; qrUrl: string; accountName: string; notice: string }) {
  ...
  return <div className="grid gap-10 xl:grid-cols-[minmax(0,1.1fr)_minmax(20rem,0.9fr)]">
    ...
    <section><p className="eyebrow">订单记录</p><ul className="mt-3 divide-y divide-line border-y border-line">{orders.map((order) => <li key={order._id} className="py-4"><button type="button" className="block w-full text-left" onClick={() => setActive(order)}><div className="flex justify-between gap-3"><span className="font-mono text-sm">{order.orderNo}</span><span className="font-mono text-xs text-muted">{statusLabel[order.status] ?? order.status}</span></div><p className="mt-2 text-sm text-muted">充值 {cny(order.paymentAmountFen)} · 到账 {cnyMicrosLabel(order.creditMicros, 2)}</p></button></li>)}{orders.length === 0 ? <li className="py-4 text-sm text-muted">还没有充值订单。</li> : null}</ul></section>
  </div>;
}
