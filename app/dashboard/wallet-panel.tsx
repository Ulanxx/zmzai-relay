...
const statusVariant: Record<string, "success" | "warning" | "danger" | "accent" | "outline"> = { pending: "warning", submitted: "accent", completed: "success", rejected: "danger", expired: "outline" };
...
export function WalletPanel({ products, initialOrders, qrUrl, accountName, notice }: { products: Product[]; initialOrders: Order[]; qrUrl: string; accountName: string; notice: string }) {
  ...
  return <div className="grid gap-10 xl:grid-cols-[minmax(0,1.1fr)_minmax(20rem,0.9fr)]">
    <section>
      <p className="eyebrow flex items-center gap-2"><Icon name="wallet" size={13} className="text-accent" />选择额度包</p>
    ...
    <section><p className="eyebrow flex items-center gap-2"><Icon name="receipt" size={13} className="text-accent" />订单记录</p><ul className="mt-3 divide-y divide-line border-y border-line">{orders.map((order) => <li key={order._id} className="py-4"><Button type="button" variant="ghost" className="h-auto w-full justify-start rounded-none px-0 py-2 text-left" onClick={() => setActive(order)}><div className="flex w-full justify-between gap-3"><span className="font-mono text-sm">{order.orderNo}</span><Badge variant={statusVariant[order.status] ?? "outline"} size="sm">{statusLabel[order.status] ?? order.status}</Badge></div><p className="mt-2 text-sm text-muted">充值 {cny(order.paymentAmountFen)} · 到账 {cnyMicrosLabel(order.creditMicros, 2)}</p></Button></li>)}{orders.length === 0 ? <li className="py-4 text-sm text-muted">还没有充值订单。</li> : null}</ul></section>
  </div>;
}
