...
export default async function BillingPage() {
  ...
  return (
    <RelayShell role="user" userName={user.name} isAdminUser={user.role === "admin"}>
      <p className="eyebrow">额度支持</p>
      <h1 className="headline mt-2 text-4xl">先用 ¥1 体验额度</h1>
      <p className="mt-3 max-w-2xl text-ink/70">每个账号首次进入会自动获得 ¥1 体验额度。余额用尽后，请联系牧之申请增加额度。</p>
      <div className="mt-8 max-w-xl">
        <MovingBorder duration={7} backgroundColor="var(--color-surface)" borderRadius="12px">
          <div className="p-6">
            ...
            <p className="mt-4 text-sm leading-6 text-muted">请附上你的登录邮箱或用户昵称，以及需要增加的额度。人工确认后会直接写入你的个人钱包。</p>
          </div>
        </MovingBorder>
      </div>
    </RelayShell>
