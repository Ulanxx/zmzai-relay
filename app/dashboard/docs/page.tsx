...
export default async function DocsPage() {
  ...
  await connectMongo();
  const models = await ModelPriceModel.find({ enabled: true, model: { $in: supportedModels } }).sort({ model: 1 }).lean();
  return (
    <RelayShell role="user" userName={user.name} isAdminUser={user.role === "admin"}>
      ...
      <p className="mt-3 text-ink/70">模型名称与上游一致。推理强度使用 <code className="font-mono text-accent-readable">reasoning_effort</code> 传入；DeepSeek 的思考开关可随请求体透传。</p>
  ...
  -d '{"model":"deepseek-v4-flash","reasoning_effort":"high","messages":[{"role":"user","content":"你好"}]}'`}</span>
          </Terminal>
        </div>
      </div>
    </RelayShell>
